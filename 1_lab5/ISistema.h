...
public:
  ...
  agregarMenu(char codigoMenu,
              string descripcion) = 0; // devuele una coleccion de DtComun
  virtual void seleccionarProductoComun(char codigoComun, int cantProducto) = 0;
  virtual void agregarProductoComun(char codigoComun, string descripcion,
                                    float precio) = 0;
  ...
  virtual void
  listarMedioTransporte() = 0; // devuelve una coleccion de DtMedioTransporte
  ...
  /*------ VENTA A DOMICILIO ------*/
  virtual bool ventaDomicilio(
      string telefono) = 0; // Returns true if client exists, false otherwise
  virtual IDictionary *
  listarProductos() = 0; // Returns a collection of DtProducto
  virtual void agregarProductoPedido(char codigo, int cantidad) = 0;
  virtual ICollection *
  listarRepartidores() = 0; // Returns a collection of DtRepartidor
  virtual void asignarRepartidorDomicilio(int idRepartidor) = 0;
  virtual DtFacturaDomicilio confirmarPedido() = 0;

  // virtual ICollectible *listarParaAgregar(int idMesa) = 0; // devuele una
  // coleccion de DtProducto virtual void seleccionarProductoAgregar(char
  // codigo, int cantidad) = 0; virtual void confirmarAgregarProducto() = 0;
  // virtual void ingresarMesa(int idMesa) = 0;
  // virtual ICollectible *productosVenta() = 0;                    // devuele
  // una coleccion de DtProducto virtual void seleccionarProductoQuitar(char
  // codigo, int cant) = 0; virtual void quitarProductoVenta() = 0; virtual void
  // finalizarVenta(int nroMesa) = 0; virtual void aplicarDescuento(int
  // descuento) = 0; virtual DtFactura generarFactura(DtVenta venta) = 0;
  // virtual ICollectible *mostrarProductos() = 0;                   // devuele
  // una coleccion de DtProducto
